'''
The purpose of this section is to extend ResNet in an ensemble approach. It designed for combining 2 models with the same number of nodes on last Fc layer before output layer
...
if OPTION == "SUM":
    ...
    out = Dense(2, activation='softmax')(addition_layer)
    ...
    model1_layer = model1(inputs)
    model2_layer = model2(inputs)
    avg_layer = Average()([model1_layer, model2_layer]) #concatenation layer
    # dense1 = Dense(final_layer_size, activation='relu')(addition_layer)
    # dense2 = Dense(final_layer_size/2, activation='relu')(dense1)
    out = Dense(2, activation='softmax')(avg_layer)
    model = Model(inputs = inputs, outputs = out)
    ...
    # dense1 = Dense(final_layer_size, activation='relu')(concat_layer)
    out = Dense(2, activation='softmax')(concat_layer)
    model = Model(inputs = inputs, outputs = out)
